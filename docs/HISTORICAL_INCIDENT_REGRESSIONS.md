...
| 23 | Persisted incident diagnostics dirtied the coordination checkout and blocked otherwise valid finalization (#151 follow-up) | `test/bootstrap-finalize.test.ts` (`incident diagnostics in the coordination root are preserved locally without blocking finalization`) | Sanitized incident diagnostics are valid local workflow evidence; the default diagnostics directory is ignored by Git so automatic workflows preserve it locally without treating it as source-tree residue. |
