...
| 23 | Persisted incident diagnostics dirtied the coordination checkout and blocked otherwise valid finalization (#151 follow-up) | `test/bootstrap-finalize.test.ts` (`incident diagnostics in the coordination root are committed before finalization`) | Sanitized incident diagnostics are valid workflow evidence; when they are the only coordination-root residue, finalization commits and pushes them instead of treating them as unrelated unsafe work. |
