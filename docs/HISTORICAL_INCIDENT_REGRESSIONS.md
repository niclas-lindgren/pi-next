...
| 23 | Persisted incident diagnostics dirtied the coordination checkout and blocked otherwise valid finalization (#151 follow-up) | `test/bootstrap-finalize.test.ts` (`incident diagnostics in the coordination root are committed before finalization`) | Sanitized incident diagnostics are valid workflow evidence and remain tracked, not Git-ignored; they are committed as soon as they are written (`runSingleIssueLifecycle`) and, as a backstop, before finalization (`commitIncidentDiagnosticsBeforeFinalization`) instead of treated as unrelated unsafe work. |
| 24 | 2026-08-25 bootstrap #83 passed implementation/verification, integrated the candidate, committed diagnostics, then failed post-integration reverification because the coordination root was dirty (#160) | `historical: post-integration reverification dirty coordination root recovered mechanically`; also `test/bootstrap-finalize.test.ts` (`already-integrated finalization reverifies away from a dirty root and preserves local work`) and `test/checkpoint.test.ts` (`production promotion recovery runs required post-integration checks before closing an already integrated candidate with a dirty root`) | Exact integrated-main reverification runs in an isolated exact-SHA workspace and finalization recovery consumes no implementation/repair/model rerun merely because the operator root contains residue. |
| 25 | 2026-08-25 bootstrap #160 implementation worker timed out after producing a dirty candidate whose required checks passed (#161) | `historical: bootstrap worker timeout left verified dirty candidate`; also `test/bootstrap-self-host.test.ts` timed-out candidate recovery/resume regressions | Worker protocol timeout metadata is retained, but a substantive candidate delta plus deterministic verification may advance without relaunching implementation solely for a nicer terminal assistant result. |
| 26 | 2026-08-26 bootstrap #163 refused to start at preflight because sanitized incident diagnostics written by a prior failed run dirtied the coordination checkout | `test/bootstrap-self-host.test.ts` (`incident diagnostics in coordination root are committed and bootstrap proceeds from preflight`) | Sanitized incident diagnostics are valid workflow evidence and are committed/pushed as soon as they are written (preflight, before the dirty-root gate); unrelated dirty paths still fail closed and are preserved. |
| 27 | CI-only `pushUnrelatedMainCommit` non-fast-forward failures on GitHub Actions (#163) | `test/bootstrap-finalize.test.ts` (`#163 fixture bare remote HEAD stays on main when Git's default branch is not main`) | Test fixtures must not inherit runner-specific `init.defaultBranch`; scratch clones must start from `main`, not an unborn non-main branch that creates an unrelated root commit before pushing to `main`. |
