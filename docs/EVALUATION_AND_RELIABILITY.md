...
### Multi-source skill registry and lazy resolver decisions (issue #116)

- **adopt-pattern** — declarative pinned dependency managers separate a reviewed install manifest from runtime selection. `skills/manifest.json` now represents one or more pinned upstream sources, each with an immutable revision, allowlist, destination, license/provenance, overlays, and independent per-source `PROVENANCE.json`. Adding/updating a source stays an explicit reviewed repository change; a worker cannot trigger network installation during normal dispatch.
- **adopt-pattern** — deterministic policy routers give registry entries a methodology/category identity and resolve one canonical discipline per axis. The kernel `skill-registry` resolver classifies role/task/path/risk into mandatory/automatic/explicit tiers, returns selected ids with source/provenance/tier/reason plus a registry fingerprint, and rejects competing methodologies (for example Matt `diagnosing-bugs` + Superpowers `systematic-debugging`) at configuration time rather than asking the worker to reconcile them.
...
### Production worker shell capability boundary (issue #162)

- **adopt-pattern** — sandboxed worker harnesses expose positive capabilities rather than a raw shell plus text blacklist. Bootstrap and production Pi workers now share a single worker shell policy that parses one command, rejects chaining/wrappers/nested shells/interpreter eval forms before process creation, limits Git to hardened read-only inspection subcommands, withholds GitHub CLI authority, allowlists mutable-worker tools explicitly, blocks built-in write/edit access to Git metadata and runtime authority/result paths, disables hooks for worker-triggered Git mutations, and runs repository-controlled build/test launchers in a detached no-`.git` execution copy inside an OS mount/network sandbox with Git/GitHub credentials/transports scrubbed and Git/GH wrappers for child processes. Missing OS sandbox support is a fail-closed launcher failure, not permission to fall back to unsandboxed repository execution.
- **reject** — treating regex denylisting of model-controlled `sh -c` strings as an enforceable security boundary for controller-owned promotion, main-branch mutation, or issue closure.
