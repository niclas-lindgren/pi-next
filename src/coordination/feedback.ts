...
export class FeedbackReporter {
  ...
  /** Restore bounded sanitized events without re-publishing them. */
  restore(events: readonly FeedbackEvent[]): void {
    for (const event of events.slice(-MAX_EVENTS)) {
      if (!event || typeof event.fingerprint !== "string") continue;
      this.counts.set(event.fingerprint, Math.min(999, (this.counts.get(event.fingerprint) || 0) + 1));
      if (event.attempt >= this.policy.recurringThreshold || this.policy.immediateSeverities.includes(event.severity)) {
